...
from models import UserVKTinder, SearhPair, SearhPairPhoto
...
def parse_users_vk_list(id_vktinder_user, searh_users):
    ...
    info_list = VKAPIusers.get_vktinder_user(id_vktinder_user) #получение данных о пользователе приложением из вк апи по его id(функция вызывается из модуля vk_api_my)
    ...
    for user in searh_users: #преобразование пользователя в экземпляра класса SearhPair
        info = SearhPair(
                ...
                user_vktinder_id = users.user_vktinder_id # здесь происходит связывание искомого человека с тем кто его ищет (один ко многим)
                )
...
def get_searh_pair_info(connection, user_vktinder_id): 
    ...
    querys = connection.query(SearhPair.searh_pair_name, SearhPair.searh_pair_surname, SearhPair.searh_pair_page_link,
                              SearhPairPhoto.photo_1, SearhPairPhoto.photo_2, SearhPairPhoto.photo_3).join(SearhPair).join(UserVKTinder)
    querys = querys.filter(UserVKTinder.user_vktinder_id == user_vktinder_id).all()
    return querys
    session.close()

def generator_get_searh_pair_info(connection, user_vktinder_id):
...
# searh_users = user1.searh_users() #вызов метода поиска к пользовательлю 1
